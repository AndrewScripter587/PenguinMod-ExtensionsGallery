    ...
    (async function(Scratch) {
        ...
        class WASMModuleType {
            ...
            constructor(source, compileOptions = {builtins: ["js-string"],importedStringConstants: "string_constants"}) {
                source ??= 0;
                const importObject = {
                ...
                if (source instanceof vm.agBuffer.Type) {
                    this.modulePromise = WebAssembly.compile(source.arrayBuffer,compileOptions)
        ...
        class WASMInstanceType {
            ...
            customId = "agWASMInst";
            static bindImports(source, target){
                const result = {};
                for (const key of Object.keys(source)) {
                    if (typeof source[key] === "function") {
                        result[key] = source[key].bind(target);
                    } else {
            ...
            *waitForFinish(promiseHandler) {
                ...
                if (this.instance?.exports) return this;
                    ...
                    if (typeof this.instancePromise.then === "function") {
        ...
        class Extension {
            ...
            getInfo() {
                return {
                    ...
                    {
                        opcode: "newModuleOptions",
                        text: "create new module [SOURCE] with compile options [OPTIONS]",
                        ...agWASM.ModuleBlock,
                        arguments: {
                            SOURCE: vm.agBuffer?.Argument ?? {shape: BlockShape.SQUARE},
                            OPTIONS: vm.dogeiscutObject?.Argument ?? {blockShape: BlockShape.PLUS},
                        }
                    },
                    ...
                    {
                        opcode: "setDebugBool",
                        text: "set debug option [OPTION] to [VALUE]",
                        blockType: BlockType.COMMAND,
                        arguments: {
                            OPTION: {type: ArgumentType.STRING, menu: 'booleanDebug'},
            ...
            getCompileInfo() {
                return {
                    ir: {
                        ...
                        newModuleOptions: (generator, block) => {
                            generator.script.yields = true
                            return {
                                kind: 'input',
                                source: generator.descendInputOfBlock(block, 'SOURCE'),
                                compileOptions: generator.descendInputOfBlock(block, 'OPTIONS'),
                            }
                        },
                        ...
                        setDebugBool: (generator, block) => {
                            
                            return {
                                kind: 'stack',
                                option: block.fields.OPTION.value,
                                value: generator.descendInputOfBlock(block, 'VALUE'),
                    ...
                    js: {
                        newModule: (node, compiler, imports) => {
                            return new imports.TypedInput(`yield* (new vm.agWASM.ModuleType(${compiler.descendInput(node.source).asUnknown()}).waitForFinish(typeof waitPromise !== "undefined" ? waitPromise : function*(promise) {return yield* vm.agWASM.util.waitPromise(promise,globalState)}))`, imports.TYPE_UNKNOWN);
                        },
                        newModuleOptions: (node, compiler, imports) => {
                            return new imports.TypedInput(`yield* (new vm.agWASM.ModuleType(${compiler.descendInput(node.source).asUnknown()}, vm.dogeiscutObject.Type.toObject(${compiler.descendInput(node.compileOptions).asUnknown()}).toJSON()).waitForFinish(typeof waitPromise !== "undefined" ? waitPromise : function*(promise) {return yield* vm.agWASM.util.waitPromise(promise,globalState)}))`, imports.TYPE_UNKNOWN);
                        },
                        ...
                        setDebugBool: (node, compiler, imports) => {
                            compiler.source += `vm.agWASM.debug["${node.option}"] = ${compiler.descendInput(node.value).asBoolean()};\n`
                        },
